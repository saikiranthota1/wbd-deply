...
const AdPage = () => {
  ...
  async function fetchData() {
    setLoading(true)
    await axios.get(`${process.env.REACT_APP_BACKENDURL}/ads/requests`).then(async (response) => {
      setPendingAds(response.data);
      await axios.get(`${process.env.REACT_APP_BACKENDURL}/ads/newrequests`).then((response) => {
        setPendingAds(response.data);
      });
    });
    await axios.get(`${process.env.REACT_APP_BACKENDURL}/ads/getads`).then((response) => {
  ...
  const handleSubmitWeight = async () => {
    try {
      setShowPopup(false);
      setLoading(true)
      await axios.post(`${process.env.REACT_APP_BACKENDURL}/ads/add/${selectedAdId}`, { weight }).then(async (response) => {
  ...
  const handleDeleteReq = async (adId) => {
    ...
    setLoading(true)
    await axios.delete(`${process.env.REACT_APP_BACKENDURL}/ads/delete/request/${adId}`).then(async(response) =>{
      setLoading(false)
    await fetchData()
  })
  }
  const handleDeleteAd = async (adId) => {
    ...
    setLoading(true)
    await axios.delete(`${process.env.REACT_APP_BACKENDURL}/ads/delete/${adId}`).then(async(response) =>{
      setLoading(false)
    await fetchData()
  })
  }
                ...
                {(activeTab === 'requests' ? pendingAds : existingAds).map((ad) => (
                  <tr key={ad.id} className="hover:bg-gray-50 transition duration-200">
                    ...
                    <td className="p-3 border-b">
                      <a
                        href={`${process.env.REACT_APP_BACKENDURL}/uploads/ads/${ad.AdImgUrl}`}
                        target="_blank"
                        rel="noopener noreferrer"
                      >
                        <img
                          src={`${process.env.REACT_APP_BACKENDURL}/uploads/ads/${ad.AdImgUrl}`}
