...
export default function Component() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const { data } = await axios.post(`${process.env.REACT_APP_BACKENDURL}/auth/register`, { username, password, email })
      alert(data.message)
  ...
  const handleKYCSubmit = async (e) => {
    ...
    setIsLoading(true);
    try {
      const response = await axios.post(`${process.env.REACT_APP_BACKENDURL}/auth/kyc`, {
        ...kycDetails,
        user: Cookies.get('user') // Include user ID from cookies
      });
        ...
        <div className="rounded-2xl overflow-hidden shadow-2xl bg-white">
          ...
          <div className="p-8 space-y-6">
                  ...
                  {Object.entries(kycDetails).map(([key, value]) => (
                    (key !== 'user' && key !== 'profile_pic') && (
                      <div key={key} className="space-y-2">
                        <label htmlFor={key} className="block text-sm font-medium text-[#424242]">
            ...
            <div className="text-center mt-4">
              <Link to={'/login'} className="text-sm text-[#229799] hover:underline">
