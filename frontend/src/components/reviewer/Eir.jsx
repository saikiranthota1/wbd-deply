...
const EIRApplications = () => {
  ...
  const fetchEIRs = async () => {
    ...
    try {
      const response = await axios.get(`${process.env.REACT_APP_BACKENDURL}/review/${reviewer}/reviews`);
  ...
  const handleSubmitReview = async (appId) => {
    ...
    try {
      await axios.post(`${process.env.REACT_APP_BACKENDURL}/review/eir/${appId}/${reviewer}`, {
        rating,
        comment
      });
